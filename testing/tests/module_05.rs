...
#[tokio::test]
async fn test_05() -> Result<(), Box<dyn std::error::Error>> {
    ...
    hc.do_get("/config").await?.print().await?;
    hc.do_get("/track").await?.print().await?;
    hc.do_get("/track").await?.print().await?;
    hc.do_get("/track").await?.print().await?;
    hc.do_get("/metrics").await?.print().await?;    
    hc.do_get("/track").await?.print().await?;
    hc.do_get("/db/users").await?.print().await?;
    hc.do_get("/me").await?.print().await?;
